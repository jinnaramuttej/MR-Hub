...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "The official innovation hub of Malla Reddy University. Programs, mentors, and capital to turn student ideas into funded companies." },
    ],
  }),
  component: Home,
});
          ...
          {ecosystemActions.map((card) => (
            ...
            >
                ...
                <div className="px-8 pb-10 pt-12">
                    <h3 className="font-display text-2xl tracking-tight text-ink group-hover:text-brand transition-colors">
                    {card.title}
                    </h3>
        ...
        >
          ...
          >
            <div className="flex -space-x-4">
                {[1,2,3,4].map(i => (
                    <div key={i} className="h-12 w-12 rounded-full border-4 border-ink bg-secondary overflow-hidden">
    ...
    <section className="py-8 border-b border-border bg-secondary/10">
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        <div className="flex flex-wrap justify-center lg:justify-between items-center gap-10 opacity-40 grayscale transition-all hover:opacity-100 hover:grayscale-0">
          {partners.map((n) => (
            <span key={n} className="font-display text-xs font-bold uppercase tracking-widest text-ink">
              {n}
            </span>
          ))}
        </div>
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-10 grid sm:grid-cols-2 lg:grid-cols-4 gap-12">
        {[
          ...
          >
            ...
            <p className="font-display text-4xl text-ink tracking-tighter">{k}</p>
            <p className="mt-2 text-sm font-bold text-muted-foreground uppercase tracking-widest">{l}</p>
          </motion.div>
        ))}
      </div>
    </section>
          ...
          {boardMembersData.map((member, index) => (
            <motion.div
              ...
              className={`group relative overflow-hidden transition-all duration-500 ${
                index === 2 
                ? "rounded-[2.5rem] bg-ink h-[450px] shadow-2xl scale-105 z-10 border-4 border-brand/20" 
                : "rounded-3xl bg-card border border-border h-[380px] hover:border-brand/40"
              }`}
            >
              <img
                src={member.image}
              ...
              <div className="absolute bottom-0 inset-x-0 p-8">
                ...
                <p className="text-xl font-display text-white group-hover:text-brand transition-colors">
                  {member.name}
                </p>
              </div>
            </motion.div>
    ...
    <section className="py-24 lg:py-32 border-b border-border bg-ink relative overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-10 relative z-10">
        <div className="text-center mb-20">
          ...
          <h2 className="font-display text-4xl lg:text-6xl tracking-tight text-white">Wall of Love<span className="text-brand">.</span></h2>
        </div>
          ...
          {testimonials.map((t, i) => (
            ...
            >
              <div className="flex gap-1 mb-8 text-brand">
                {[1, 2, 3, 4, 5].map(s => <Sparkles key={s} size={14} className="fill-brand" />)}
              </div>
              <p className="text-xl text-white font-medium leading-relaxed italic">"{t.quote}"</p>
              <div className="mt-10 flex items-center gap-5">
                ...
                <div>
                  <p className="font-bold text-white group-hover:text-brand transition-colors">{t.name}</p>
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-10 mb-16">
          ...
          <Link to="/programs" className="group flex items-center gap-3 text-sm font-black uppercase tracking-widest text-ink hover:text-brand transition-colors">
            Full Curriculum <div className="h-10 w-10 rounded-full border border-border grid place-items-center group-hover:border-brand transition-colors"><ArrowRight size={16} /></div>
          </Link>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {programs.map((p, i) => (
            ...
            >
                ...
                >
                <span className="text-4xl font-display text-brand/20 group-hover:text-brand transition-colors">0{i + 1}</span>
                <h3 className="mt-8 font-display text-2xl text-ink">{p.name}</h3>
                <p className="mt-2 text-xs font-bold text-muted-foreground uppercase tracking-tighter">{p.duration} · {p.format}</p>
                <p className="mt-6 text-muted-foreground leading-relaxed">{p.blurb}</p>
                <div className="mt-10 flex items-center justify-between border-t border-border pt-8">
                    <span className="text-xs font-black uppercase tracking-widest text-brand group-hover:translate-x-2 transition-transform block">Learn More</span>
        ...
        <div className="grid lg:grid-cols-2 gap-20 items-center">
          <motion.div {...fadeInUp}>
            ...
            <div className="mt-12 flex flex-wrap gap-5">
              <Link
                to="/events"
                className="px-10 py-5 rounded-full bg-brand text-white font-bold hover:scale-105 transition-all shadow-[0_0_40px_rgba(249,115,22,0.4)]"
              >
                Register Now
              </Link>
            </div>
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-10 mb-16">
          ...
          <Link to="/explore" className="group flex items-center gap-3 text-sm font-black uppercase tracking-widest text-ink hover:text-brand transition-colors">
            All Live Deals <div className="h-10 w-10 rounded-full border border-border grid place-items-center group-hover:border-brand transition-colors"><ArrowRight size={16} /></div>
          </Link>
        </div>
              ...
              >
                <div className="flex items-center gap-8">
                  <div className="flex flex-col items-center justify-center h-16 w-16 rounded-2xl bg-secondary text-brand border border-brand/10 group-hover:bg-brand group-hover:text-white transition-colors">
                    <span className="text-[10px] font-black uppercase">{e.date.split(' ')[0]}</span>
                    <span className="text-xl font-black leading-none">{e.date.split(' ')[1]?.replace(',', '') || ''}</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-black uppercase tracking-widest text-brand mb-1 block">{e.format}</span>
                    <h4 className="font-display text-xl text-ink">{e.title}</h4>
                  </div>
                </div>
    ...
    <section className="py-24 lg:py-32 border-b border-border">
      <div className="mx-auto max-w-7xl px-6 lg:px-10 grid lg:grid-cols-12 gap-20 items-center">
        <div className="lg:col-span-5">
          ...
          <Link
            to="/investors"
            className="mt-10 inline-flex items-center gap-3 rounded-full bg-ink text-white px-8 py-4 font-bold transition-all hover:scale-105 shadow-xl"
          >
    ...
    <section className="py-24 lg:py-32 border-b border-border bg-ink/[0.01]">
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        <div className="flex items-end justify-between gap-10 mb-16">
          <h2 className="font-display text-4xl text-ink">In The Press<span className="text-brand">.</span></h2>
          <Link to="/about" className="text-sm font-black uppercase tracking-widest text-ink hover:text-brand transition-colors">Archive</Link>
        </div>
        <div className="space-y-4">
          {news.map((n, i) => (
            <motion.div 
            ...
            >
              <p className="col-span-12 lg:col-span-2 text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground">{n.source}</p>
              <p className="col-span-12 lg:col-span-8 font-display text-xl lg:text-2xl text-ink group-hover:text-brand transition-colors">{n.title}</p>
              <p className="col-span-12 lg:col-span-2 text-sm text-muted-foreground lg:text-right font-medium">{n.date}</p>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-10 text-center relative">
        ...
        >
          ...
          <div className="flex flex-wrap justify-center gap-6">
            ...
            <Link
              to="/about"
              className="px-12 py-6 rounded-full border border-border bg-white text-ink font-bold text-lg hover:bg-secondary transition-all"
            >
              Schedule a Call
            </Link>
          </div>
